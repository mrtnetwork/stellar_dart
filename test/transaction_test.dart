...
void _transaction() {
  test('Transaction v1', () {
    ...
    final transaction = StellarTransactionV1(
      ...
      cond: PrecondTime(
        TimeBounds(minTime: BigInt.from(0), maxTime: BigInt.from(1727106100)),
      ),
      operations: [
        Operation(
  ...
  test('envelope3', () {
    ...
    expect(
      transaction.operations[0].body.operationType,
      OperationType.changeTrust,
    );
    expect(
      transaction.operations[1].body.operationType,
    ...
    final verify = pubkey.verify(
      digest: payload.txHash(),
      signature: decode.signatures[0].signature,
    );
    expect(verify, true);
  });
}
